...
func NewConverterWorker(
	client client.Client, fileUtil onlyoffice.OnlyofficeFileUtility, server *config.ServerConfig,
...
func (c ConverterWorker) UploadFile(ctx context.Context, payload []byte) error {
	...
	tctx, span := tracer.Start(ctx, "convert")
	defer span.End()

	var msg request.ConvertJobMessage
	if err := json.Unmarshal(payload, &msg); err != nil {
		logger.Errorf("could not parse convert job message. Reason: %s", err.Error())
		return err
	}

	c.logger.Debugf("got a new file %s conversion job (%s)", msg.FileID, msg.UserID)
	...
	if _, err := srv.Files.Insert(&drive.File{
		...
		FileExtension:                c.fileUtil.GetFileExt(msg.Title),
		OriginalFilename:             msg.Title,
		OwnedByMe:                    true,
		Title:                        msg.Title,
		Parents:                      file.Parents,
		MimeType:                     mime.TypeByExtension(fmt.Sprintf(".%s", c.fileUtil.GetFileExt(msg.Title))),
	}).Media(body).Do(); err != nil {
		c.logger.Errorf("could not modify file %s: %s", msg.FileID, err.Error())
		return err
	}

	c.logger.Debugf("successfully modified file %s", msg.FileID)
